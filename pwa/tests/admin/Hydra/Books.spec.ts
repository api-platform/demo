...
test.describe('Books admin with Hydra', () => {
  ...
  test('Create a book', async ({ page }) => {
    ...
    // Save a review iri
    const reviewIri = await page.locator('td.column-reviews').filter({ hasNotText: 'No results found' }).locator('li').nth(0).textContent()
  ...
  test('Delete a book', async ({ page }) => {
    await page.goto('/admin#/books?page=2')
